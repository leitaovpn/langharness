...
#: Supplies the candidate values for one command's first argument.
ArgumentSource = Callable[[str], Iterable[str]]
...
class PaletteCompleter(Completer):
    ...
    def get_completions(
        ...
        # than helpful.
        wanted = rest.lower()
        for value in source(rest):
            if value.lower().startswith(wanted):
                yield Completion(value, start_position=-len(rest))
